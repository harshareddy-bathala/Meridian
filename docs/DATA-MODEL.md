...
>
> **Post-reception tables** — for the reception verdict, loss diagnosis, the station health watch, owner reports and the evidence dataset — are described under their own heading below and are **planned, not built**. Nothing here is a migration; the rules they share are D-101.
...
Six tables for modules 13–17. None exists, and the column tuples are the intent, settled finally when each stage writes its migration. What they share is decided in D-101:
...
- **Simulator ground truth is in none of them.** An injected fault's cause lives in the simulator's own run record and is joined only at evaluation (D-102).
...
`kind` is `pass` (with `assignment_id`) or `weekly` (with `period_start`); `channel` is `email` or `telegram`. The rendered text is not stored — `template_version` and the stored results it read regenerate it, and `content_sha256` proves the regeneration matches. **No recipient address is held in this table**; where one is held at all is D-104's open question. A simulated station's report is rendered and recorded like any other and delivered only to a test sink, never to a person.
...
| `loss_diagnoses.cause` *(planned)* | `satellite_silent`, `station_not_listening`, `obstruction`, `interference`, `timing_fault`, `undetermined` — D-101 |
| `report_deliveries.kind` *(planned)* | `pass`, `weekly` — D-095 |
| `report_deliveries.channel` *(planned)* | `email`, `telegram` — D-095 |
