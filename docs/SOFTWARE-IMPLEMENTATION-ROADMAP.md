...
Stage 26 adds the reception verdict's label here, as D-103 settles it. Stage 27's diagnosed causes reuse these labels' names where the two overlap rather than coining a second vocabulary, and Stage 30 builds on this manifest rather than defining another.
...
Stage 25 adds observation-sourced rows from each reception's noise floor. Settle the unit here: the documented `noise_floor_dbm` presumes a calibration this roadmap excludes (D-101).
...
Added with Stages 25–30 (D-092). Those stages follow this one only so that no existing stage number moves; the project is not software-complete until their gates pass too.
...
Stages 25–30 add the post-reception layer — modules 13–17 in the submission list — decided on 2026-09-13 in D-091 to D-105. They are numbered after Stage 24 so that no stage other documents reference is renumbered, and they are listed here in dependency order:
...
This stage gives the later ones their evidence and their ground truth. Depends on Stages 9, 13, 19 and 21, and on the team's answer to D-100.
...
If D-100 is accepted, a `spec(msp):` change adds the optional `noise_floor_dbfs`, `receiver_gain_db`, `snr_samples` and `decode` fields as MSP 0.3, with conformance tests, and **merges before anything below** (`GIT-WORKFLOW.md` Rule 9). If it is not, skip this subsection: Stages 26–28 run on MSP 0.2's evidence in the reduced forms D-100 lists, and the interference cause is always undetermined.
...
- write each injected cause, target, onset and parameters to the run record beside the seed — **never into an MSP body and never into a platform table** (D-102);
...
Module 13. Depends on Stages 15, 17 and 25, and on D-103 settling what "usable" means.
...
Implement D-103's answer in Stage 15's outcome labelling, independent of every verdict input.
...
- trained and evaluated on measured receptions only (D-078, D-102);
...
In `meridian.prediction`. Reads observations and listening evidence through their modules' interfaces, never an MSP body. A pass's own verdict is never a feature of that pass's yield prediction (D-099).
...
Diagnose every failed or partial reception, and every held assignment whose window passed with no observation. Never an `expired` assignment — that is a decline (D-008, D-101).
...
Decide in `meridian.reliability` whether a decoded reception below the partial threshold counts as captured for SC-4, and write it into the miss classification. The verdict informs that rule; it does not make it (D-099).

### Evidence boundary

Meridian's own data only at runtime — no archive lookup (D-099, D-105). A simulated reception is never evidence about a measured station.
...
Module 16. Depends on Stages 26, 27 and 28, and on D-104 — built to its option (b), team-operated stations only, until the team decides.
...
Create `platform/notifications`, and add the `notifications` scope to `GIT-WORKFLOW.md` Rule 3 and the CI `conventions` pattern in the same change (D-099).
...
- versioned; no language model anywhere (D-095);
...
As D-104 decides. No address in `report_deliveries`, in logs, or in any MSP message.
...
Create `platform/datasets`, add the `datasets` scope as Stage 29 did, and move Stage 15's manifest and hashing into it so there is one implementation, not two (D-099).
...
Measured and simulated receptions in separate files; simulated ones exported only when requested by name (D-101).
