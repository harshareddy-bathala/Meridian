...
The microcontroller station is a **LoRa satellite receiver**, proving MSP on small hardware. It is not a field sensor product, and the telemetry in the fallback above is the station reporting on itself (D-093). Its LoRa telemetry passes are also the second data type the post-reception layer in §5.6 must handle.
...
Where each capability lives in the platform is D-099, what it stores D-101, and the build order roadmap Stages 25–30.
...
**We say what happened to each reception, with a calibrated confidence and a named cause.** Other networks already rate receptions — SatNOGS does, manually and by automatic rules. What Meridian adds is narrower: an automatic confidence whose stated probabilities are tested against what happened, and an automatic attribution of cause for every loss, scored against known causes (D-096).

### What we do not claim

- **That nobody else assesses receptions**, or that any part of this is a first. Receptions are rated elsewhere; our claim is calibration and automatic cause attribution, and nothing more (D-096).
- **That loss diagnosis is accurate in the field.** Its confusion matrix is measured on simulated faults whose causes are known, and shows that evidence of a fault's shape is attributed to that fault. Real labelled cases are reported separately, however few (D-102).
...
SC-7 cannot be measured until the team settles what "usable" means (D-103), and SC-8 and SC-9 are simulated results, labelled as such wherever they are reported.
...
| First detection varies with horizon, obstruction and link margin far more than the sub-second orbital effect SC-3 measures | High | Medium | Measurement tested on archive data before SC-3 relies on it; timing from the Doppler curve noted as an alternative (D-097) |
| No label for a "usable" reception independent of the verdict's own inputs | Medium | Medium | Settled before the verdict is built; SC-7 is not measured until it is (D-103) |
| Simulator faults and the loss diagnosis, written by one team, agree by construction | Medium | Medium | Fault effects specified first and reviewed by another member; real labelled cases reported separately (D-102) |
...
Owner reports (§5.6) need a way to reach a station's owner, and a contact address is personal data. How — or whether — one is held is open (D-104). Until the team decides, reports go only to stations the team itself operates, and the statement above holds as written.
...
The post-reception layer in §5.6 changes nothing here: it needs no hardware beyond Tiers 1 and 2, and the request stays at ₹27,600 (D-091).
...
**Nothing in this section is planned work.** It has no roadmap stage, no success criterion and no line in the budget (D-098). Here "Phase 1" means everything this project commits to, and "Phase 2" what might follow it — **not** §12's Phase 1 (Foundations) and Phase 2 (Intelligence), which are committed build phases.
...
**Every alert type needs an external validation source before it is built.** That source scores alerts offline and never sits on the path that raises one, so the independence test still holds. A way of reaching people who receive alerts would be personal data, and inherits D-104's question.
