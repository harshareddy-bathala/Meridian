...
``meridian.api.msp`` binds the endpoints of MSP §8. Every error response is
...
**Bound:** all four endpoints of MSP §8 — ``GET /time``, ``POST /register``,
``POST /heartbeat`` and ``POST /observations``, one module each under
``meridian.api.msp``.
...
a requirement of the protocol rather than a platform convention, because the
integrity of every reported figure depends on simulated and measured data never
being aggregated.
"""
